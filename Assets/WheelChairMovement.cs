...
public class WheelChairMovement : MonoBehaviour
{
    ...
    private Vector3 startPosition; // For 3rd version of wheelchair movement
    ...
    public void StartMoveForward(float distance)
    {
        if (!isMoving)
        {
    ...
    public IEnumerator turn(float angle)
    {
        ...
        while (Mathf.Abs(totalRotation) < Mathf.Abs(angle))
        {
            float step = direction * turnSpeed * Time.deltaTime;
